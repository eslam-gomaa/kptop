...
class customDashboardMonitoring(PrometheusNodeMetrics):
    ...
    def build_variables(self, variables):
        ...
        variables += self.default_cli_args
        for var in variables:
                if var['cliArgument']['enable']:
                    parser.add_argument(
                        ...
                        default=var['default'],
                        help=f'Specify the {var["name"]} variable value - default: "{var["default"]}"'
                    )
    ...
    def build_custom_dashboard(self, yaml_file):

        # Parse the file
        parse_dashboard = self.parse_dashboard_yaml_file(yaml_file)
        if not parse_dashboard['success']:
            logging.error(f"Failed to load dashboard file: '{yaml_file}'")
            logging.error([parse_dashboard['fail_reason']])

        # Build the Layout structure
        self.make_layout(layout_structure_dct=parse_dashboard['data'])

        # Build the dashboard variables
        self.build_variables(variables=parse_dashboard['data'].get('dashboard').get('variables', {}))

        # vistualize the metrics on the layout
        self.update_layout_visualization(layout_structure_dct=parse_dashboard['data'])
    ...
    def make_layout(self, layout_structure_dct, print_layout=False) -> Layout:
        ...
        # elif left_enable and not right_enable and not middle_enable:
        #     layout["body"].split_row(Layout(name="left", size=layout_dct['body']['boxes']['left']['size']))
        #     self.layout_list = ['left']

        # elif right_enable and not left_enable and not middle_enable:
        #     layout["body"].split_row(Layout(name="right", size=layout_dct['body']['boxes']['right']['size']))
        #     self.layout_list = ['right']

        # elif middle_enable and not left_enable and not right_enable:
        #     layout["body"].split_row(Layout(name="middle", size=layout_dct['body']['boxes']['middle']['size']))
        ...
        if found_variables:
            for variable in found_variables:
                if variable[1:] in self.variables:
                    ...
                    updated_query = query.replace(variable, self.variables[variable[1:]])
